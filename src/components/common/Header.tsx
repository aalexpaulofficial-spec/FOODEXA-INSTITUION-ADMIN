...
import {
  ...
  CreditCard, Lock, Globe, Wifi, WifiOff
} from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 h-16 bg-[#09090B]/80 dark:bg-[#09090B]/80 backdrop-blur-xl saturate-[180%] border-b border-zinc-800/50 px-4 sm:px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg" title={isRealtimeConnected ? t('common.connected') : t('common.disconnected')}>
          {isRealtimeConnected ? (
          ...
          <span className={`text-[10px] font-semibold ${isRealtimeConnected ? 'text-emerald-400' : realtimeStatus === 'CHANNEL_ERROR' || realtimeStatus === 'TIMED_OUT' ? 'text-red-400' : 'text-amber-400'}`}>
            {isRealtimeConnected ? t('common.connected') : realtimeStatus === 'CHANNEL_ERROR' || realtimeStatus === 'TIMED_OUT' ? t('common.disconnected') : t('common.reconnecting')}
          </span>
        </div>
        ...
        <div className="relative" ref={langRef}>
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl hover:bg-zinc-800/60 text-zinc-400 hover:text-zinc-200 transition-all border border-zinc-800/60"
            title="Language"
          >
            <Globe className="w-3.5 h-3.5" />
            <span className="text-[11px] font-bold">{LANG_OPTIONS.find(l => l.code === language)?.short || 'EN'}</span>
            <ChevronDown className={`w-3 h-3 transition-transform ${showLangDropdown ? 'rotate-180' : ''}`} />
          </button>

          {showLangDropdown && (
            <div className="absolute right-0 mt-2 w-40 bg-[#0C0C0E] border border-zinc-800 rounded-2xl shadow-2xl py-1.5 z-50 animate-fade-in-down">
              ...
              {LANG_OPTIONS.map((lang) => (
                ...
                >
                  <span className="flex items-center gap-2.5">
                    <span className="font-mono text-[10px] w-5 text-center">{lang.short}</span>
