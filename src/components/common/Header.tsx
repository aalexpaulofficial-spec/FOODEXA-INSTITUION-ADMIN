...
import {
  Search, Bell, Sparkles, ShieldAlert, ChevronDown, Building2, Clock,
  LogOut, Sun, Moon, Laptop, Menu, X, QrCode, Smartphone
} from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const displayName = currentPortal === 'super_admin' ? 'Super Admin' : (currentInstitution?.contactPerson || user?.email?.split('@')[0] || 'Admin');
  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-[#09090B] border-b border-zinc-800/50 px-3 sm:px-6 flex items-center justify-between text-[#FAFAFA]">
      <div className="flex items-center space-x-2 sm:space-x-4 lg:space-x-6">
        ...
        {currentPortal !== 'super_admin' ? (
          <div className="hidden sm:flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-zinc-900/80 border border-zinc-800 text-xs font-medium text-zinc-300">
            ...
            <span className="max-w-[120px] md:max-w-[180px] truncate">{currentInstitution?.name || 'Loading...'}</span>
        ...
        <div className="relative">
          ...
          {showThemeDropdown && (
            <div className="absolute left-0 mt-2 w-44 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl py-1.5 z-50 text-xs">
              ...
              {(['light', 'dark', 'system'] as const).map((mode) => (
                <button
                  ...
                  className={`w-full px-3 py-2 text-left flex items-center justify-between transition-colors ${
                    theme === mode
                      ? mode === 'light' ? 'bg-amber-500/10 text-amber-400 font-bold'
                        : mode === 'dark' ? 'bg-indigo-500/10 text-indigo-400 font-bold'
                ...
                >
                  <span className="flex items-center space-x-2">
                    ...
                    {mode === 'system' && <Laptop className="w-3.5 h-3.5 text-emerald-400" />}
      ...
      <div className="hidden lg:flex items-center flex-1 max-w-md mx-8">
        <div onClick={onOpenAISearch} className="relative w-full cursor-pointer group">
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
            <span className="px-1.5 py-0.5 rounded bg-zinc-800 text-[10px] font-mono text-zinc-400">Ctrl+K</span>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 bg-[#0C0C0E] border border-zinc-800 rounded-2xl shadow-2xl py-3 px-4 z-50 text-xs">
              <div className="flex items-center justify-between pb-2 border-b border-zinc-800 mb-2">
                <span className="font-bold text-zinc-200">Notifications</span>
              </div>
        ...
        <div className="relative">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xs font-bold text-white shadow-sm">
              {currentPortal === 'super_admin' ? 'SA' : 'IA'}
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-xs font-semibold leading-none text-white">{displayName}</p>
              <p className="text-[10px] text-zinc-500 leading-tight truncate">{displayEmail}</p>
            </div>
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-[#0C0C0E] border border-zinc-800 rounded-xl shadow-2xl py-2 z-50 text-xs">
              <div className="px-3 py-2 border-b border-zinc-800">
                <div className="font-bold text-zinc-200">{displayName}</div>
                <div className="text-[10px] text-zinc-500 font-mono">{displayEmail}</div>
              </div>
              <button
                onClick={onLogout}
